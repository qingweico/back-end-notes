...
![JVM](https://img.imgdb.cn/item/6034a2145f4313ce252eb92a.png)
...
优点: 当对象被移动(垃圾回收时移动对象很普遍)时, 只需改变句柄池中到对象实例对象的指针的值, 而reference本身不需要修改 
```
...
一般来说, JIT编译出来的机器码性能比解释器高, C2编译器启动时长比C1编译器慢, 系统稳定执行后, C2编译器执行速度远远快于C1编译器
