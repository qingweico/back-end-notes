...
<font style="color:purple;font-size:20px">一定要返回上级目录才可以正常运行,因为一个类名的全称是包名+类名,所以当编译过后JVM会按照当前目录下寻找字节码文件(如果没有配置CLASSPATH路径),而如果不返回上级目录,则会路径重复,JVM会报出找不到或无法加载主类的错误</font>
