...
面向消息的中间件(MOM: Message Oriented Middleware)是指利用高效可靠的消息传递机制进行与平台无关(跨平台)的数据交流,并基于数据通信来进行分布式系统的集成,通过提供消息传递和消息排队模型在分布式环境下提供应用的解耦、弹性伸缩、冗余存储、流量削峰、异步通信、数据同步等功能
...
MQ的消费模式可以分为两种:一种是push,一种是pull(push模式也是基于pull)

- push: 主动推送消息给客户端,时效性好,但是缺点就是如果客户端没有做好流量限制,一旦服务端推送大量消息,会导致客户端消息积压甚至崩溃
